...
function item() { return { id: "lead-1", board: { id: "board-1" }, column_values: [{ id: "state", text: "TX", type: "text" }, { id: "phone", text: PHONE, type: "phone" }] }; }
...
function config(overrides = {}) {
  ...
  return { fake, get queries() { return queries; }, value: {
    expectedWebhookToken: TOKEN, canonicalBoardId: "board-1", stateColumnId: "state", phoneColumnIds: ["phone"], stateSource: "sales_board_business_state",
...
test("Sales Board audit association does not require or invent recording consent", async () => {
  const run = await running();
  const result = await post(run.url);
  assert.equal(result.response.status, 202);
  assert.equal(JSON.stringify(run.fake.calls).includes("Permit Recording"), false);
