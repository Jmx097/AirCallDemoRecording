...
- `audit-only-receiver.mjs` is a local loopback-only (`127.0.0.1` or `::1`) HTTP receiver composed from the pure event boundary, read-only injected Monday adapter, and injected decision store service. It is **unprovisioned, unstarted, not deployed, and not webhook-registered**. It has no runtime environment loader, no recording path, no Aircall recording-client import, and never owns or closes the supplied store. Public ingress and provider webhook registration remain separate approval gates. Any contemplated use requires live staging acceptance first; this module is not an operational integration.
