...
test("runtime config requires safe required env, fixes mapping and loopback", () => {
  ...
  assert.deepEqual(config.phoneColumnIds, ["phone_mkqkk6nv", "phone_mkqk71tf"]); assert.equal(config.ruleset.version, "audit-only-disabled-v1"); assert.equal(config.ruleset.states.TX, false);
...
test("Monday transport accepts only fixed query documents and returns bounded redacted shapes", async () => {
  ...
  const result = await query({ query: MONDAY_CALLBACK_READ_ONLY_QUERIES.native, variables: { itemId: "1", columnIds: ["dropdown_mm5ht9fz"] } });
